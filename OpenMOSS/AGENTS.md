...
| Task | Location | Notes |
|------|----------|-------|
| Backend API dev | `app/` | FastAPI routers, services, models |
| Frontend dev | `webui/src/` | Vue 3 views, components, stores |
| Agent prompts | `prompts/` | 4 role prompts (planner/executor/reviewer/patrol) |
| Agent skills | `skills/` | CLI tool + 8 skill packages |
...
|--------|------|----------|------|
| `app` | FastAPI | `app/main.py` | Entry point, route registration, SPA serving |
| `config` | Singleton | `app/config.py` | YAML config loader |
| `init_db` | Function | `app/database.py` | SQLAlchemy table creation |
| `get_current_agent` | Dependency | `app/auth/dependencies.py` | API key auth |
...
cd webui && npm run build && cp -r dist/* ../static/
