...
def create_task(
    db: Session, name: str, description: str = "", type: str = "once"
) -> Task:
    """创建任务"""
...
def update_task(
    db: Session, task_id: str, name: str = None, description: str = None
) -> Task:
...
def create_module(
    db: Session, task_id: str, name: str, description: str = ""
) -> Module:
    """创建模块"""
    task = db.query(Task).filter(Task.id == task_id).first()
    if not task:
...
def update_task_readme(
    ...
    task.readme_generated = generated
...
def increment_retry_count(db: Session, task_id: str, retry_type: str) -> Task:
    """增加重试计数"""
    task = db.query(Task).filter(Task.id == task_id).first()
    if not task:
        raise ValueError(f"任务 {task_id} 不存在")
    ...
    if retry_type == "plan":
        current = int(task.plan_retry_count or "0")
        task.plan_retry_count = str(current + 1)
    elif retry_type == "readme":
        current = int(task.readme_retry_count or "0")
        task.readme_retry_count = str(current + 1)
